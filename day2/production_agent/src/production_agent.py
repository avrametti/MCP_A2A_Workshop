...
A FastAPI-based A2A (Agent-to-Agent) compliant server that monitors Press 103
manufacturing data, providing equipment status, OEE metrics, and downtime analysis.

Architecture:
    - MQTT: Subscribe to Press 103 topics, cache to production_cache.json
    - MySQL: Connection pool to mes_lite database for historical data
    - A2A Protocol: Standard agent discovery and messaging endpoints
"""
...
# =============================================================================
# PYDANTIC MODELS - A2A MESSAGE PROTOCOL
# =============================================================================
...
# =============================================================================
# PYDANTIC MODELS - AGENT DISCOVERY
# =============================================================================
...
class Capabilities(BaseModel):
    streaming: bool
    pushNotifications: bool = Field(alias="pushNotifications")
...
def get_oee_rating(oee: float) -> str:
    """Get OEE rating based on percentage."""
    if oee > 0.85:
        return "World Class"
    elif oee > 0.60:
        return "Acceptable"
...
async def startup_event():
    """Initialize MQTT and MySQL connections."""
    logger.info("=" * 50)
    logger.info("Starting Production Agent - Press 103")
    logger.info("=" * 50)
...
def get_equipment_status_data() -> dict[str, Any]:
    ...
    speed_percent = (speed_val / setpoint_val) * 100 if setpoint_val > 0 else 0.0
...
def get_oee_summary_data() -> dict[str, Any]:
...
def get_downtime_summary_data(hours_back: int = 24) -> dict[str, Any]:
...
# ROUTING LOGIC
# =============================================================================
def route_message_to_skill(message_text: str) -> str:
    """Route message to appropriate skill based on keywords."""
    ...
    if any(keyword in text_lower for keyword in ["downtime", "down", "stopped", "reason", "why"]):
        return "get_downtime_summary"

    # Default to equipment status
    return "get_equipment_status"
...
async def send_message(request: MessageRequest):
    ...
    Routes to appropriate skill based on message content keywords.
    """
    ...
    skill_name = route_message_to_skill(message_text)

    # Execute skill
    timestamp = datetime.utcnow().isoformat() + "Z"
    if skill_name == "get_equipment_status":
        skill_data = get_equipment_status_data()
    elif skill_name == "get_oee_summary":
        skill_data = get_oee_summary_data()
    elif skill_name == "get_downtime_summary":
        skill_data = get_downtime_summary_data()
    else:
        skill_data = get_equipment_status_data()
